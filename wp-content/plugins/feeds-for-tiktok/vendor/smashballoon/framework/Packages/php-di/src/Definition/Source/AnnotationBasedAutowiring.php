...
use TikTokFeeds\Vendor\DI\Annotation\Inject;
use TikTokFeeds\Vendor\DI\Annotation\Injectable;
use TikTokFeeds\Vendor\DI\Definition\Exception\InvalidAnnotation;
use TikTokFeeds\Vendor\DI\Definition\ObjectDefinition;
use TikTokFeeds\Vendor\DI\Definition\ObjectDefinition\MethodInjection;
use TikTokFeeds\Vendor\DI\Definition\ObjectDefinition\PropertyInjection;
use TikTokFeeds\Vendor\DI\Definition\Reference;
use TikTokFeeds\Vendor\Doctrine\Common\Annotations\AnnotationRegistry;
use TikTokFeeds\Vendor\Doctrine\Common\Annotations\Reader;
use TikTokFeeds\Vendor\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use TikTokFeeds\Vendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'TikTokFeeds\Vendor\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'TikTokFeeds\Vendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('TikTokFeeds\Vendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'TikTokFeeds\Vendor\DI\Annotation\Injectable');
